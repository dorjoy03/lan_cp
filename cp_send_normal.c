...
static int send_file(uint64_t size, int fd, int sockfd) {
...
int main(int argc, char *argv[]) {
    ...
    ret = statx(AT_FDCWD, "./file10G1", 0, STATX_MODE | STATX_SIZE, &statx_buf);
    if (ret != 0) {
        perror("Failed to statx");
        return 1;
    }

    ret = send_file(statx_buf.stx_size, fd, sockfd);
    if (ret == -1) {
